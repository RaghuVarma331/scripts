...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    ...
    brew install protobuf==3.18.0 xz brotli lz4 aria2 detox coreutils p7zip gawk
    PIP=pip
fi

sudo "$PIP" install backports.lzma extract-dtb protobuf==3.18.0 pycrypto docopt zstandard twrpdtgen
echo -ne '\n' | ./13.sh
